  ...
  return (
    ...
    <nav className="fixed z-10 bg-white w-full">
      <div className="border-b border-b-[#e7ebf0] p-4 flex justify-between items-center">
        ...
        <div className="hidden lg:flex gap-4">
          <div className=" pl-0 pr-3 sm:pl-10 gap-4 flex items-center">
            <ul className="font-light gap-8 flex">
              <li
                onMouseOver={() => handleDropdown("socialstep")}
                className="flex gap-2 items-center cursor-pointer"
              >
                <div>Social Steps</div>
                <i
        ...
        <div className="p-5">
          <ul className="flex flex-wrap gap-6 bg-[#fffefa]">
            {subNavData[0].subItem.map((item) => (
              ...
              >
                <li key={item.name} className="w-64 p-2 hover:bg-gray-100 rounded-xl">
                  <div className="flex gap-3 font-semibold items-center">
                    <div className="w-16 h-16 text-white flex justify-center items-center">
                      <img
                        src={item.icon}
      ...
      <div
        ...
        ref={ref}
        className={`transition-all duration-150 ${
          company ? "translate-y-0 visible" : "opacity-0 h-0 invisible translate-y-4"
        } absolute left-[19rem] top-[4.6rem] w-2/5 rounded-xl px-6 py-2 bg-[#fffefa] border border-x-gray-300 border-r-gray-300 border-l-gray-300`}
      >
        <div>
          <ul className="flex gap-8 flex-wrap py-3 bg-[#fffefa]">
            {subNavData[2].subItem.map((item) => (
              ...
              >
                <li key={item.name} className="p-2 w-64 hover:bg-gray-100 rounded-xl">
                  <div className="flex gap-5 items-center">
                    <div className="h-16 w-16 flex justify-center items-center">
      ...
      >
        <div>
          <ul className="">
            {subNavData[3].subItem.map((item) => (
              <li key={item.name} className="pb-4 hover:text-gray-500">
    ...
    <nav className="block lg:hidden relative z-20">
      <div className="border-b border-b-[#e7ebf0] p-4 flex justify-between items-center">
        ...
        <button onClick={handleNavbar} className="border-none outline-none">
          {navbar ? (
            <i className="fa-solid fa-xmark"></i>
          ) : (
            <i className="fa-solid fa-bars"></i>
      ...
      <div
        ref={mobilenavbarRef}
        className={`transition-all duration-300 flex flex-col p-3 bg-white overflow-hidden ${
          navbar ? "max-h-screen" : "max-h-0"
        }`}
      >
        <ul>
          <li>
            <button
              onClick={() => handleSubNavbar("Outlet type")}
              className="flex gap-2 items-center w-full justify-between border-b border-b-[#e7ebf0] py-3"
            >
              Outlet type
              <i
                className={`fa-solid ${
            ...
            {dropdown === "Outlet type" && (
              <div className="pl-5 py-3">
                <ul>
                  {subNavData[0].subItem.map((item) => (
                    <li key={item.name} className="hover:text-gray-500">
                      <NavLink to={item.slug}>{item.name}</NavLink>
                    </li>
                  ))}
                </ul>
          ...
          <li>
            <button
              onClick={() => handleSubNavbar("Features")}
              className="flex gap-2 items-center w-full justify-between border-b border-b-[#e7ebf0] py-3"
            >
              Features
              <i
          ...
          <li className="border-b border-b-[#e7ebf0] py-3">
            <NavLink to="/pricing">Pricing</NavLink>
          </li>
          <li>
            <button
              onClick={() => handleSubNavbar("Social Steps")}
              className="flex gap-2 items-center w-full justify-between border-b border-b-[#e7ebf0] py-3"
            >
              Social Steps
              <i
                className={`fa-solid ${
                  dropdown === "Social Steps"
                    ? "fa-chevron-up"
                    : "fa-chevron-down"
                }`}
              ></i>
        ...
        <div className="flex flex-col gap-4 mt-6">
          <button className="border-none bg-transparent text-gray-700">
            Sign in
          </button>
          <button
            className={`${
